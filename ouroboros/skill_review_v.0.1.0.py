...
docs/ARCHITECTURE.md describes runtime boundaries, host-service surfaces,
extension dispatch, skill state, grants, and the invariant that skills must not
mutate the self-modifying repository.
docs/DEVELOPMENT.md describes engineering standards for reviewed changes,
bounded execution, no silent truncation, and native-risk extension dispatch.
BIBLE.md is the constitutional tie-breaker for agency, continuity, immune
integrity, reviewability, and version-history discipline.

Skills must not write to the self-modifying repo.
Skills must request secrets through grants.
Skills must keep state under skill state dir.
Skills must not exfiltrate secrets.
Skills must expose bounded tools/routes/widgets.
